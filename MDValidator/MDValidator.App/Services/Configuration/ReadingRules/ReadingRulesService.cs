...
using DomainReadingRules = MDValidator.Domain.Configuration.ReadingRules;

namespace MDValidator.App.Services.Configuration.ReadingRules
{
    public class ReadingRulesService : RulesBase, IReadingRulesService
    {
        public HeadersViewModel ProcessHeaderFile(string fileName)
        {
            ...
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Open(fileName, false))
            {
                ...
                string requiredTemplate = "READID TABLE HIERARCHY";
                int totalColumns = 3;
                ...
                {
                    DomainReadingRules.Header header = new DomainReadingRules.Header()
                    {
                        ReadId = row[0],
                        Table = row[1],
                    ...
                    if (string.IsNullOrEmpty(header.ReadId) || string.IsNullOrWhiteSpace(header.ReadId) ||
        ...
        public ItemsViewModel ProcessItemsFile(string fileName)
        {
            ...
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Open(fileName, false))
            {
                ...
                string requiredTemplate = "READID STEP CONTROL TABLE FIELD COMMAND PARAM_TABLE PARAM_FIELD PARAMETER CONNECTOR";
                int totalColumns = 10;
                ...
                foreach (var row in rows)
                {
                    ...
                    DomainReadingRules.Item item = new DomainReadingRules.Item()
                    {
                        ReadId = row[0],
                    ...
                    //check for completeness
                    if (string.IsNullOrEmpty(item.ReadId) || string.IsNullOrWhiteSpace(item.ReadId) ||
                        item.Step < 1 ||
